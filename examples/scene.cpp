...
int main() {
    ...
    res->append("font2", "SourceHanSansCN-Normal.otf", Resource::Font);
    ...
    Font font2("font2", 96.0);
    font2.setFontColor(StdColor::DarkOrange);

    auto text_layer = new Layer("text_layer");
    text_layer->append(1, font.textToSprite("text1", "Hello EasyEngine!", engine.painter()));
    font2.setFontSize(32.f);
    text_layer->append(2, font2.textToSprite("text2", "请按下 Q 键以进入另一场景，或直接点击按钮进入下一场景", engine.painter()));
    text_layer->sprite(1)->properties()->position = {370, 100};
    text_layer->sprite(2)->properties()->position = {320, 300};
